...
class RepNCSPELAN4Drop(nn.Module):
    # csp-elan
...
class RepNCSPELAN4(nn.Module):
    # csp-elan
    def __init__(self, c1, c2, c3, c4, drop, c5=1):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        if drop == 1:
            drop = 0.5
        ...
        self.c = c3 // 2
        self.cv1 = Conv(c1, c3, 1, 1)
        self.cv2 = nn.Sequential(RepNCSP(c3 // 2, c4, c5), Conv(c4, c4, 3, 1), nn.Dropout(drop))
        self.cv3 = nn.Sequential(RepNCSP(c4, c4, c5), Conv(c4, c4, 3, 1), nn.Dropout(drop))
        self.cv4 = Conv(c3 + (2 * c4), c2, 1, 1)
